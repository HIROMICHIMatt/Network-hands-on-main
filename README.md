...
11. オンプレの仮想マシンからアプリケーションにアクセスできるか確認する。しかしこの状態ではできないため、Hub-Spoke間のPeeringの設定を変更する。

12. **HubVNet**リソースへと移動。**Peering**をクリックして、既存のPeeringのどちらかをクリック。

    ![](images/20211014-screenshot1.png)

13. **仮想ネットワーク ゲートウェイまたはルート サーバー**の設定を、**この仮想ネットワークのゲートウェイまたはルート サーバーを使用する** に変更して、**保存**をクリック。

    ![](images/20211014-screenshot2.png)

14.もう片方のPeeringに対しても同様の設定を実施。

15. 以下のようになっていることを確認。

    ![](images/20211014-screenshot3.png)

16. **SpokeVNet1**に移動。**Peering**にて既存のPeeringをクリック。

    ![](images/20211014-screenshot4.png)

17. **仮想ネットワーク ゲートウェイまたはルート サーバー**の設定を、**リモート仮想ネットワークのゲートウェイまたはルート サーバーを使用する** に変更して、**保存**をクリック。

    ![](images/20211014-screenshot5.png)

18. **SpokeVNet2** に対しても同様の設定を行う。

19. 設定完了後、再度手順11を行い、アプリケーションへのアクセスを確認する。
